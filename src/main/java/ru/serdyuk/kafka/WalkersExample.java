 ...
 *
 * $ java -cp practice-5.0.0-standalone.jar ru.serdyuk.kafka.WalkersExample
 *
 ...
 *
 */
public class WalkersExample {

  public static void main(final String[] args) throws Exception {
    final String bootstrapServers = args.length > 0 ? args[0] : "localhost:9092";
    final Properties streamsConfiguration = new Properties();
    streamsConfiguration.put(StreamsConfig.APPLICATION_ID_CONFIG, "walkers-example");
    streamsConfiguration.put(StreamsConfig.CLIENT_ID_CONFIG, "walkers-example-client");
    ...
    final KTable<String, Long> count = textLines
      .flatMapValues(value -> Arrays.asList(pattern.split(value.toLowerCase())))
      .groupBy((key, word) -> word)
      .count();

    count.toStream().to("walkTooMuch", Produced.with(stringSerde, longSerde));
